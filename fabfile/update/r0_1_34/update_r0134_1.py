...
from fabric.api import env, execute, task, run, cd, get
...
@task
def update_host(bootstrap_path=None, release=None, skip_clone=True, deployment_pip_dir=None,
                map_area=None, bootstrap_filename=None, bootstrap_branch=None, **kwargs):
    """
    Example:

        brew update && fab -H localhost update.update_host:bootstrap_path=/Users/erikvw/source/bcpp/fabfile/conf/,release=develop,use_branch=True,bootstrap_branch=develop,skip_pip_download=True,skip_clone=True
...
@task
def update_centralserver(**kwargs):
    """
    ...
    update(conf_filename=conf_filename, **kwargs)


@task
def update_nodeserver(**kwargs):
...
def update(skip_db=None, bootstrap_filename=None, venv_name=None, map_area=None, **kwargs):
